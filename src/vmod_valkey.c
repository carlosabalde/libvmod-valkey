    ...
    VCL_STRING password)
{
    ...
    if (protocol == enum_vmod_valkey_RESP3) {
        VALKEY_LOG_ERROR(ctx,
            "%s is not supported",
            "RESP3");
        return;
    ...
    VCL_BOOL ignore_slaves, VCL_INT max_cluster_hops)
{
    ...
    if ((connection_timeout >= 0) &&
        ...
        unsigned clustered = type == enum_vmod_valkey_cluster;
...
{
    if ((location != NULL) && (strlen(location) > 0) &&
        ((!db->cluster.enabled || type == enum_vmod_valkey_cluster))) {
        // Initializations.
        vcl_state_t *config = vcl_priv->priv;
        enum VALKEY_SERVER_ROLE role = type2role(type);
...
EASY_EXEC(vmod_db_easy_execute, arg_vmod_valkey_db_easy_execute);
EASY_EXEC(vmod_db_easy_execute_proxy, arg_vmod_valkey_easy_execute);
    ...
    VCL_STRING prometheus_extra_labels)
{
    ...
    Lck_Lock(&db->mutex);
    if (format == enum_vmod_valkey_json) {
        AZ(VSB_printf(vsb,
            "{"
                "\"servers\": {"
                    "\"total\": %" PRIu64 ","
    ...
    } else if (format == enum_vmod_valkey_prometheus) {
        // Beware:
...
VCL_VOID
vmod_easy_execute(VRT_CTX, struct arg_vmod_valkey_easy_execute *args)
{
...
type2role(VCL_ENUM type)
{
    ...
    if (type == enum_vmod_valkey_master) {
        result = VALKEY_SERVER_MASTER_ROLE;
    } else if (type == enum_vmod_valkey_slave) {
        result = VALKEY_SERVER_SLAVE_ROLE;
    } else if (type == enum_vmod_valkey_auto) {
        result = VALKEY_SERVER_TBD_ROLE;
    } else if (type == enum_vmod_valkey_cluster) {
        result = VALKEY_SERVER_TBD_ROLE;
    } else {
...
parse_protocol(VCL_ENUM protocol)
{
    ...
    if (protocol == enum_vmod_valkey_default) {
        result = VALKEY_PROTOCOL_DEFAULT;
    } else if (protocol == enum_vmod_valkey_RESP2) {
        result = VALKEY_PROTOCOL_RESP2;
    } else if (protocol == enum_vmod_valkey_RESP3) {
